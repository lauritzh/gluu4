...
Since the *standard* authentication flow will be different, the `casa` script contents must be updated. Expand the row corresponding to Casa script, back up the current script contents and then replace with those [here](https://github.com/GluuFederation/casa/raw/version_4.3.0/plugins/account-linking/extras/casa.py).
